...
- [稳健] 图床上传等待响应超时时返回明确提示，不再输出整段通用异常 traceback
- [稳健] 默认请求超时时间从 60 秒提高到 120 秒，降低大图/慢速压缩场景的误超时概率
- [工程] 新增图床 API 超时分流回归测试

### v1.2.4

- [严重] 修复图片段 `url` 为本地/非 HTTP 地址时被误当成可上传链接，导致“不支持的图片地址”的问题
- [严重] 修复 OneBot/NapCat 回复图片时平台名不为 `aiocqhttp` 就不调用 `get_msg`，导致“请发送/回复一张图片”的问题
- [工程] 新增非 HTTP 图片 URL base64 兜底与 OneBot 回复取图回归测试

### v1.2.3

- [严重] 修复图床 HTTP 400 时真实错误信息被吞，只显示状态码的问题
- [稳健] `/图床上传` 从消息段提取到图片 URL 时先本地下载，再以文件方式上传，减少 QQ/NapCat 临时图片链接被图床服务器拉取失败的问题
- [工程] 新增 HTTP 错误体透传与消息图片 URL 本地上传路径的回归测试

### v1.2.2

- [安全] 修复 Telegram 回复取图下载失败时，含 bot token 的下载 URL 可能进入 AstrBot 日志的问题
- [稳健] 图床 API 成功响应格式异常时不再抛出未捕获异常，改为返回友好提示
- [工程] 新增敏感 URL 日志脱敏与响应格式化异常的回归测试
